...
class _DiscoverResultsScreenState extends State<DiscoverResultsScreen> {
  ...
  List<MapEntry<Scheme, RecommendationResult>> _getFilteredSchemes(AppProvider provider) {
    ...
    return provider.recommendedSchemes.where((entry) {
      ...
      if (!titleMatch) {
        ...
        for (var word in words) {
          if (word.length > 3 && (scheme.name.toLowerCase().contains(word) || scheme.category.toLowerCase().contains(word))) {
            titleMatch = true;
            break;
          }
        }
      ...
      if (_selectedType != 'All') {
        if (!scheme.benefits.toLowerCase().contains(_selectedType.toLowerCase())) return false;
      }
      if (_selectedBenefit != 'All') {
        if (!scheme.benefits.toLowerCase().contains(_selectedBenefit.toLowerCase())) return false;
      }
      if (_selectedMinistry != 'All') {
        if (!scheme.sponsoringBody.toLowerCase().contains(_selectedMinistry.toLowerCase())) return false;
      }
  ...
  Widget _buildDropdownChip(String label, List<String> options, String currentValue, ValueChanged<String> onChanged) {
    return Container(
            ...
            child: CircleAvatar(
              ...
              child: Image.asset(
                ...
                errorBuilder: (context, error, stackTrace) {
                  return const Icon(Icons.android, color: Color(0xFF2563EB), size: 20);
                },
              ),
            ),
          )
              ...
              child: Column(
                ...
                children: [
                    ...
                    child: Row(
                      children: [
                        ...
                        OutlinedButton.icon(
                          ...
                          icon: const Icon(Icons.filter_list, size: 14, color: Color(0xFF2563EB)),
                          label: Text(
                            "Filter",
                            style: GoogleFonts.inter(
                              fontSize: 11.5,
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        const Icon(Icons.assignment_outlined, color: Color(0xFF2563EB), size: 28),
                        ...
                        Flexible(
                          child: ElevatedButton(
                            onPressed: () {
                              Navigator.of(context).push(
                                MaterialPageRoute(
                                  builder: (_) => const SaarthiWelcomeScreen(), // Redirects to Saarthi setup/profile
                                ),
                              );
                            },
                      ...
                      children: [
                        CircleAvatar(
                          ...
                          child: Image.asset(
                            ...
                            width: 32,
                            height: 32,
                            errorBuilder: (context, error, stackTrace) => const Icon(Icons.android, color: Colors.white),
                          ),
                        ),
                        const SizedBox(width: 12),
                        Expanded(
