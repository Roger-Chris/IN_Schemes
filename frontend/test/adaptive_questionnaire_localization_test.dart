...
void main() {
  ...
  group('Adaptive Questionnaire Multilingual Localization Tests', () {
    ...
    test(
      '1. QuestionNode.toLocalized produces 100% Tamil node for Tamil language',
      () {
        ...
        expect(
          locTa.questionText,
          equals('உங்கள் தற்போதைய கல்வித் தகுதி என்ன?'),
        );
        ...
        expect(
          locEn.questionText,
          equals('What is your current level of education?'),
        );
    ...
    test(
      '2. QuestionNode fallback to CentralizedTranslator when Tamil fields are null',
      () {
        const rawNode = QuestionNode(
          id: 'test_q2',
    ...
    test(
      '3. Questionnaire UI Control buttons & labels translate properly to Tamil',
      () {
        expect(
          translator.translate('Skip to Results'),
          equals('முடிவுகளுக்குச் செல்'),
        );
    ...
    test('4. Category and Ministry titles translate properly', () {
      ...
      expect(
        translator.translateTag('Women Entrepreneurship'),
        contains('பெண்'),
      );
      expect(
        translator.translateTag('Ministry of Finance'),
        contains('அமைச்சகம்'),
      );
