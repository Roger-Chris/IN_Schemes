...
void main() {
  group('CentralizedTranslator Priority Pipeline & Protection Rules', () {
    ...
    test(
      'Preserves protected text: URLs, Emails, Phone Numbers, IDs, and Dates',
      () {
        expect(
          translator.isProtectedText('https://msme.gov.in/scheme'),
          isTrue,
        );
    ...
    test('Translates chips, tags, levels, and categories into Tamil', () {
      expect(
        translator.translateTag('Central Scheme'),
        equals('மத்திய அரசு திட்டம்'),
      );
      expect(
        translator.translateTag('State Scheme'),
        equals('மாநில அரசு திட்டம்'),
      );
      ...
      expect(
        translator.translateTag('Conditional'),
        equals('நிபந்தனைக்குட்பட்டது'),
      );
    });

    test(
      'Translates dynamic content phrases, compound strings, and micro-texts',
      () {
        expect(
        ...
        expect(translator.translate('Candidate'), equals('விண்ணப்பதாரர்'));
        expect(translator.translate('Varies'), equals('மாறுபடும்'));
      },
    );

    test(
      'LocalizedScheme priority pipeline prefers JSON Tamil name over translation',
      () {
        ...
        expect(
          localizedTa.name,
          equals('தமிழ்நாடு குறு சிறு தொழில்கள் கடன் திட்டம்'),
        );
    ...
    test(
      'LocalizedScheme falls back to CentralizedTranslator when nameTa is empty',
      () {
        const rawScheme = Scheme(
          id: 'test_2',
          schemeCode: 'IN_002',
        ...
        expect(
          localizedTa.documents.first.issuingAuthority,
          contains('அதிகாரம்'),
        );
      },
    );
    ...
    test(
      'LocalizedScheme returns unmodified values when languageCode is en',
      () {
        const rawScheme = Scheme(
          id: 'test_3',
          schemeCode: 'IN_003',
