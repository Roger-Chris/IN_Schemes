...
void main() {
  ...
  group('Recommendation & Category Scheme Limits Audit', () {
    ...
    test(
      '1. Recommended For You section receives top 5 schemes limit',
      () async {
    ...
    test(
      '2. Categories return complete matching datasets without 5-item cap',
      () async {
    ...
    test(
      '3. Category Counts match exact size of getSchemesByCategory list',
      () async {
        ...
        for (final entry in counts.entries) {
          ...
          expect(
            countValue,
            equals(schemesList.length),
            reason:
                'Count for $categoryName ($countValue) must match schemes list length (${schemesList.length})',
          );
        }
      },
    );
    ...
    test(
      '4. searchSchemes returns all matching schemes without limit',
      () async {
    ...
    test(
      '5. getRecommendedSchemes without limit parameter returns all eligible recommendations',
      () async {
