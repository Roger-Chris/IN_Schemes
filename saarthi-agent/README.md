...
Saarthi is the multilingual voice guide for the IN Schemes application. LiveKit
provides realtime rooms, turn handling, noise cancellation, and cloud deployment;
Sarvam AI provides the voice pipeline:

- Speech recognition: `saaras:v3` in transcription mode with language detection
- Language model: `sarvam-105b`
- Speech synthesis: `bulbul:v3`, Tamil-first with the `kavitha` speaker

## Sarvam setup

Keep the API key out of Git. Create an ignored `.env.sarvam` file beside this
README:
...
For Hindi output, use `SARVAM_TTS_LANGUAGE=hi-IN` and a compatible speaker such
as `shubh`. For English output, use `en-IN`. Saaras can continue detecting the
input language when `SARVAM_STT_LANGUAGE=unknown`.

Update the ignored secrets and deploy with the LiveKit CLI:

```console
lk agent update-secrets --secrets-file .env.sarvam --overwrite
lk agent deploy
```

The deployed agent name is `saarthi-agent`.

## Scheme catalog

The agent searches `data/scheme_catalog.json` through its
`search_scheme_catalog` tool before answering scheme-specific questions. Search
results contain compact eligibility, benefit, application, document,
verification, and official-source fields; the full 5 MB catalog is never placed
in the LLM prompt.

To add Tamil later, populate the existing `localization.ta` fields and replace
`data/scheme_catalog.json` with the updated file. Keep the same top-level
`{"metadata": ..., "data": [...]}` structure, run `uv run pytest`, and deploy
again. Tamil names and summaries are already included in the search index.

## LiveKit starter reference
