...
from ai4bharat_600m_asr import (
    AI4BHARAT_600M_REVISION,
    AI4BharatMultilingualASR,
)
...
DEFAULT_WARMUP_AUDIO = REPOSITORY_ROOT / "voice/data/generated_audio/bench_004.mp3"
DEFAULT_TTS_CACHE = REPOSITORY_ROOT / "voice/.cache/ai4bharat-600m-demo-responses"


class AI4Bharat600MVoiceAssistant:
    def __init__(
        self,
        *,
        ...
        self.asr = AI4BharatMultilingualASR(
            device=device,
            language="ta",
            decoder=decoder,
            offline=True,
    ...
    async def _write_tts(self, text: str, output_path: Path) -> None:
        await edge_tts.Communicate(text=text, voice=self.tts_voice).save(
            str(output_path)
        )
    ...
    def respond(
        ...
        metrics: dict[str, Any] = {
            "model": "ai4bharat-indicconformer-600m-multilingual",
            "revision": AI4BHARAT_600M_REVISION[:12],
            "decoder": self.decoder,
            "device": self.asr.device,
            "providers": self.asr.execution_providers,
...
def build_demo(assistant: AI4Bharat600MVoiceAssistant) -> gr.Blocks:
    with gr.Blocks(title="நம்ம திட்டம் — AI4Bharat 600M தமிழ் குரல் சோதனை") as demo:
        gr.Markdown(
            """
            # நம்ம திட்டம் — AI4Bharat 600M தமிழ் குரல் சோதனை

            மைக்ரோஃபோனை அழுத்தி ஒரு திட்டத்தைப் பற்றி தமிழில் கேளுங்கள்.
            உதாரணம்: **“திருமண உதவித் தொகைக்கு வயது வரம்பு என்ன?”**

            > ASR உள்ளூரில் AI4Bharat IndicConformer 600M RNNT/CUDA மூலம் இயங்குகிறது.
            > பதில்கள் செயற்கை டெமோ உள்ளடக்கம்; அதிகாரப்பூர்வ அரசு வழிகாட்டல் அல்ல.
            """
        )
        microphone = gr.Audio(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the AI4Bharat 600M demo.")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=7862)
    parser.add_argument("--device", choices=("cuda", "cpu"), default="cuda")
    parser.add_argument("--decoder", choices=("ctc", "rnnt"), default="rnnt")
...
def main() -> None:
    args = parse_args()
    assistant = AI4Bharat600MVoiceAssistant(
        device=args.device,
        decoder=args.decoder,
        warmup_audio=args.warmup_audio,
    )
    print(
        f"Loaded AI4Bharat 600M {args.decoder}/{args.device} in "
    ...
    build_demo(assistant).queue(default_concurrency_limit=1).launch(
